...
function GameSystemsSelectHTML({
    ...
    const gamesystemsItems = gamesystems.map((system) => {
        return { value: system.key, label: <span>{system.description}</span> };
    });
    ...
    return (
        <Form.Item<GameUpdateItem> label={<Tooltip messageKey={tooltip}>{message}</Tooltip>} name="gamesystem" rules={[createFormRule]}>
            <Select style={{ textAlign: 'left' }} options={gamesystemsItems} onBlur={save} onSelect={save} loading={loading} disabled={disabled} />
        </Form.Item>
    );
}
