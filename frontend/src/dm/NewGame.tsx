...
export default function PostGame() {
    ...
    >
        <Form.Item<FieldType>
            label="Title"
            name="title"
            rules={[{ required: true, message: 'Please input a game title!' }]}
        >
            <Input />
        </Form.Item>
        ...
        <Form.Item<FieldType>
            label="Max Players"
            name="maxplayers"
            rules={[{ required: true, message: 'Please input max number of players' }]}
        >
            <Select defaultValue={4} options={Array.from({length: 7}, (_, i) => i + 1).map(idx => {return { value: idx, label: <span>{idx}</span> }})} />
        </Form.Item>
