...
export const up = async (uz: MigrationParams<any>) => {
    ...
    await sq.transaction(async (transaction: any) => {
        ...
        await sq.query('ALTER TABLE gamesessions RENAME TO _samesessions_old', { raw: true, transaction });
        await sq.query(
        ...
        await sq.query('UPDATE users SET lastSeenChannel = (SELECT key FROM channels = "new_members")', { raw: true, transaction });
        await qi.changeColumn(
            'users',
            'lastSeenChannel',
            {
        ...
        /* FIXME */
        await sq.query(
            'UPDATE users SET lastSeenMessage = greetingmessage.messageId FROM users INNER JOIN grretingmessage ON users.key = greetingmessage.userId',
            { raw: true, transaction }
        );
        /* FIXME */
        await qi.changeColumn(
