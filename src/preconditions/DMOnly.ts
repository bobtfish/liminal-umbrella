...
export class DMOnlyPrecondition extends Precondition {
  ...
  private async checkDM(guild: Guild, userId: string) {
    ...
    return !!Array.from(guildMember.roles.cache).map(([_, role]) => role.name).find(name => name === 'Dungeon Master' || name === 'Admin') ?
      this.ok() : this.error({ message: 'Only Dungeon Masters can use this command!' });
  }
}
