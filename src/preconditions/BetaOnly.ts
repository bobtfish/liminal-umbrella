...
export class BetaOnlyPrecondition extends Precondition {
  ...
  private async checkDM(guild: Guild, userId: string) {
    ...
    return !!Array.from(guildMember.roles.cache).map(([_, role]) => role.name).find(name => name === 'BotBetaTester' || name === 'Admin') ?
      this.ok() : this.error({ message: 'Only Beta Testers can use this command!' });
  }
}
